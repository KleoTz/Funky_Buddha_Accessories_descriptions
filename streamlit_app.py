...
# Ορίζουμε το URL ως σταθερά
URL = "https://docs.google.com/spreadsheets/d/1rP1cqvVWQsslZhJrfVxRDarHptFRCCCwkc06TPJdmDc/edit?usp=sharing"

# 1. Σύνδεση με τη σταθερή λίστα στο Google Sheets
conn = st.connection("gsheets", type=GSheetsConnection)

try:
    # Εδώ δίνουμε το URL ρητά μέσα στη read για να μην ψάχνει στα τυφλά
    df_master = conn.read(spreadsheet=URL, worksheet="Sheet1", ttl=0)
    
    # Καθαρισμός κενών στις κεφαλίδες (πολύ σημαντικό!)
    df_master.columns = [str(c).strip() for c in df_master.columns]
    
    if 'STYLE NR.' in df_master.columns:
        df_master['STYLE NR.'] = df_master['STYLE NR.'].astype(str).str.strip()
    else:
        st.error(f"Δεν βρέθηκε στήλη 'STYLE NR.'. Βρέθηκαν: {list(df_master.columns)}")
        st.stop()
